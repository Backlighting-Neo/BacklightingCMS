...
Class ACT_Code
	...
	Private Sub Class_Initialize()
 	...
 	Public Function ArticleContent(ModeID)
		...
		TempletContent = LoadTemplate(GetNodeText("templateurl"))'读取模板路径
		...
		If GetNodeText("readpoint")>0  Or GetNodeText("infopurview")="2"  Or (GetNodeText("infopurview")="0" And (ACTCMS.ACT_L(GetNodeText("classid"),19)=1 Or ACTCMS.ACT_L(GetNodeText("classid"),20)>1 Or ACTCMS.ACT_L(GetNodeText("classid"),19)=2)) Then

			  Dim Templates
			  Templates=""
			 
			  Templates=Templates& " <title>正在为您跳转到  "&GetNodeText("title")&"  www.actcms.com</title>"& vbCrLf
			  Templates=Templates& "<script type=""text/javascript"">"& vbCrLf
			  Templates=Templates& "<!--"& vbCrLf
			  Templates=Templates& " location.href='" & ACTCMS.ActCMSDM&"List.asp?C-"&ModeID&"-" &GetNodeText("id")&".Html" & "';"& vbCrLf
			  Templates=Templates& "//-->"& vbCrLf
			  Templates=Templates& "</script>"& vbCrLf
			  Call FSOSaveFile(Templates, FilePathName)
			  Exit Function
			End If
	 ...
	 If InStr(ArticleContents, "[NextPage]") > 0 Then 
			...
			TempletContent = ReplaceArticleContent(ModeID,ContentText,ArticleContentArr(I) & ArticlePageStr)
			Call FSOSaveFile(ContentText,FilePathName)
			Next
		Else
			Call FSOSaveFile(ReplaceArticleContent(ModeID,StrContent,ArticleContents),FilePathName)
  		End If 
	End Function
	...
	Public Function Loadfile(TemplateContent) 
  		 On Error Resume Next
		 TemplateContent=RegReplace(TemplateContent, "<" & "!--#include file=""(..//)*(.+?)""-->", "{file:$2}")
 	  ...
 	  Public Function ArrayToxml(DataArray,Recordset,row,xmlroot)
		...
		End Function
		 ...
		 Function ReplaceArticleContent(ModeID,TempletContent,ArticleContents)
			...
			'======逆光新增功能=======
			'用于将文章中可能的教师姓名加上链接
			'ArticleContents=ReplaceTeacherName(ArticleContents)
            '代码启用后会导致程序timeout		
			'======
			...
			End If 
			 ...
			 If InStr(TempletContent, "{$SeoTitle}") > 0  Then
				If Trim(Actcms.ACT_L(Application(AcTCMSN & "classid"),25))<>"" Then 
					TempletContent = Replace(TempletContent, "{$SeoTitle}", Actcms.ACT_L(GetNodeText("classid"),25))
				Else 
					TempletContent = Replace(TempletContent, "{$SeoTitle}", Actcms.ACT_L(GetNodeText("classid"),2))
				End If 
			 End If 
		...
		Function ACT_A_List(ClassID,ActF,ATT,ArticleSort,OpenTypeStr,ListNumber,RowHeight,TitleLen,ColNumber,TypeClassName,TypeNew,ACTIF,NavType,Nav,MoreLinkType,MoreLink,Division,DateForm,DateAlign,TitleCss,DateCss,DiyContent,ModeID,SubClass,ContentLen) 
			...
			Sqlstr="Select TOP " & ListNumber & " ID,ClassID,Title,UpdateTime,ActLink,FileName,infopurview,readpoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords"&ACTCMS.DIYField(ModeID)&" From "&ACTCMS.ACT_C(ModeID,2)&" Where " & Parameter & " isAccept=0 AND delif=0 " & ACTCMS_ATT &ACT_IF& " ORDER BY IsTop Desc,OrderID desc," & ArticleSort
 			If ActF=2 Then 
 			 ...
 			 J=1
 				For K=0 To ActNum'17
						...
						End if
   						...
   						If Trim(DIYFieldText)<>"" Then '11开始
 							Dim DIYFieldarr,jj,ii:ii=14
							DIYFieldarr=Split(DIYFieldText,",")
 							For jj=1 To UBound(DIYFieldarr)
 								 If (InStr(DiyContents,"#"&DIYFieldarr(jj))>0) and (trim(ACTSQL(ii,N))<>"") Then
									 DiyContents=Replace(DiyContents,"#"&DIYFieldarr(jj),ACTSQL(ii,N))
 								 Else
									 DiyContents=Replace(DiyContents,"#"&DIYFieldarr(jj),"")
								 End If 
								 ii=ii+1
							Next 
  						End If 
			...
			Select Case ClassID 
				...
				Case Else
					...
					Else
 					...
 					End If 
			End Select
 			 Sqlstr= "Select TOP " & ListNumber & " ID,ClassID,Title,UpdateTime,ActLink,FileName,infopurview,readpoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords"&ACTCMS.DIYField(ModeID)&" From "&ACTCMS.ACT_C(ModeID,2)&" Where " & Parameter & " isAccept=0 AND delif=0 AND picurl<>'' AND SlIDe=1  ORDER BY IsTop Desc,OrderID desc, ID Desc "
		...
		Function GetLastArticleList(ActF,ClassID,PageStyle,ArticleSort,OpenType,PageNumber,RowHeight,TitleLen,ColNumber,TypeClassName,TypeNew,NavType,Nav,Division,DateForm,DateAlign,TitleCss,DateCss,ACTIF,DiyContent,ModeID,SubClass,ContentLen) 
		 ...
		 If Application(AcTCMSN & "ACTCMS_TCJ_Type") <> "Folder" Then GetLastArticleList="该标签位置出错":Exit Function 
			...
			If ModeID="0" Then ModeID=Cint(Application(AcTCMSN & "modeid"))
			If Ucase(Left(Trim(ArticleSort),2))<>"ID" Then  ArticleSort=ArticleSort & ",ID Desc"
			   SqlStr = "SELECT ID FROM "&ACTCMS.ACT_C(ModeID,2)&" Where " & Parameter & " isAccept=0 AND delif=0  "&ACT_IF&"  ORDER BY IsTop Desc,OrderID desc," &ArticleSort 
 			   Dim RS,N
			   Set RS=ACTCMS.ACTEXE(SqlStr)
				...
				OpenTypeStr = Gopen(OpenType)
					  ...
					  For I = 1 To PageNum
						 ...
						 Next
						  ...
						  SqlStr = "SELECT ID,ClassID,Title,UpdateTime,ActLink,FileName,infopurview,readpoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords"&ACTCMS.DIYField(ModeID)&" FROM "&ACTCMS.ACT_C(ModeID,2)&"   Where ID in (" & TempIDArrStr & ") AND isAccept=0 AND delif=0  "&ACT_IF&" ORDER BY IsTop Desc,OrderID desc," &ArticleSort 
	...
	Function GetClassNavigation(ModeID, OpenType, ColNumber, NavHeight, TCss, Division,NavType,Nav,ACTF,ThisCss,DiyContent)
				 ...
				 Dim  Parameter,MoreLinkStr,ACT_IF,ACTCMS_ATT,RSs,ACT_SQL,ACT
 				  If ModeID<>"0" Then Pmode=" And ModeID="&ModeID&" "
					If  ClassID ="0" Then '查询所有
						SqlStr = "Select ClassID,ModeID,ParentID From Class_act Where  ParentID='0' AND labelfor=1  and ACTlink=1 "&Pmode&"   Order by Orderid asc,ID asc "
					ElseIf ClassID ="1" Then 
						SqlStr = "Select ClassID,ModeID,ParentID From Class_act Where  ParentID='" & Application(AcTCMSN & "classid") & "' AND labelfor=1   and  ACTlink=1 "&Pmode&"   Order by Orderid asc,ID asc"
					Else
						If InStr(ClassID, ",") > 0 Then
							SqlStr = "Select ClassID,ModeID,ParentID From Class_act Where   ClassID IN (" & ClassID & ") AND labelfor=1  and  ACTlink=1 "&Pmode&"   Order by Orderid asc,ID asc"
						Else
							SqlStr = "Select ClassID,ModeID,ParentID From Class_act Where  ClassID = '" & ClassID & "' AND labelfor=1    and  ACTlink=1 "&Pmode&"   Order by Orderid asc,ID asc"
						End If 
					End If  
				 Set RS=ACTCMS.ActExe(SqlStr)
				...
				If 	ActF=2 Then 
 						...
 						N=0
 						 For K=0 To ActNum
							 ...
							 If ATT="0" Then  ACTCMS_ATT="" Else ACTCMS_ATT = " And ATT="&ATT
 							 If Ucase(Left(Trim(ArticleSort),2))<>"ID" Then  ArticleSort=ArticleSort & ",ID Desc"
 							 Sqlstrs="Select TOP " & ListNumber & " ID,ClassID,Title,UpdateTime,ActLink,FileName,infopurview,readpoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords"&ACTCMS.DIYField(ACTSQL(1,N))&" From "&ACTCMS.ACT_C(ACTSQL(1,N),2)&"  Where " & Parameter & " isAccept=0 AND delif=0 " & ACTCMS_ATT &ACT_IF& " ORDER BY IsTop Desc,OrderID desc," & ArticleSort
				 ...
				 Set RS=ACTCMS.ActExe(SqlStr)
 				 If RS.EOF Then	 ACT_Correlation_Article="<li>暂无相关链接":RS.Close:Set RS=Nothing:Exit Function
					 If Trim(RS(0)) <> "" And IsNull(RS(0)) = False Then
					  ...
					  SqlStr = "Select TOP " & ListNumber & " ID,ClassID,Title,UpdateTime,ActLink,FileName,infopurview,readpoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords"&ACTCMS.DIYField(Application(AcTCMSN & "modeid"))&"  FROM "&ACTCMS.ACT_C(Application(AcTCMSN & "modeid"),2)&"  Where  (" & SqlKeyWordStr & ")   "&ACT_IF&" AND isAccept=0 AND delif=0 ORDER BY IsTop Desc,OrderID desc," & ArticleSort
 						If ActF=2 Then 
		...
		Function GbgPic(Division, ColSpanNum)
			 ...
			 Else
			   ...
			   GbgPic = "<tr><td Height=1"  & ColStr & " background=" & Division & " ></td></tr>"
			 End If
		End Function
	   ...
	   End Function
		...
		Function ReplaceTeacherName(ArticleContents)
			Dim TeacherNameRs,TeacherName,SpaceString,SpaceNumber,TeacherNameForID,TransTeacherName
			
			SpaceString="&nbsp; "
			Set TeacherNameRs=actcms.actexe("SELECT Article_ACT.Title, Article_ACT.ID FROM Article_ACT WHERE (((Article_ACT.ClassID)='0163041956'))")
			ArticleContents=ArticleContents+"<br>"+cstr(TeacherNameRs.RecordCount)
			Dim Counter
			Counter=0
			
			Do While Not TeacherNameRs.eof
				TeacherName=Replace(TeacherNameRs("Title")," ","")'对数据库中的教师姓名字段去空格
				Response.write TeacherName
				Counter=Counter+1
				if Counter>20 Then Response.End
				For SpaceNumber=0 to 4
					Dim i,j
					For i=1 to len(TeacherName)-1
						TransTeacherName = TransTeacherName + mid(TeacherName,i,1)
						For j=1 to SpaceNumber
							TransTeacherName = TransTeacherName +SpaceString
						Next
					Next
					TransTeacherName = TransTeacherName + mid(TeacherName,i,1)
					TeacherName=TransTeacherName
					ArticleContents="<br>"+ArticleContents+TeacherName
					If Instr(ArticleContents,TeacherName)>0 Then
						
						ArticleContents=Replace(ArticleContents,TeacherName,"<a href='List.asp?C-1-" & TeacherNameRs("ID") & ".html' target='_blank'>" & TeacherName & "</a>")
					End If
				Next
				
				TeacherNameRs.MoveNext
			Loop
		End Function
End Class
%>