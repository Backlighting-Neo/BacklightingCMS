...
ModeID=actcms.act_l(ClassID,10)
 If modeid=0 Then  
 	classtitle="最近更新新闻列表"
	 Call  rsstop()
 	response.write  listclass
 Else 
 	classtitle=actcms.ACT_L(ClassID,2)
	 Call  rsstop()
 	response.write  listarticle(ClassID)
End If 
response.write "</channel>"&vbcrlf &"</rss>"&vbcrlf
Sub rsstop()
    ...
    response.write "<?xml version=""1.0"" encoding=""utf-8""?>" & _
        ...
        vbcrlf&"<channel>" & _
        vbcrlf&"  <title><![CDATA["&actcms.ActCMS_Sys(0)&"-"&classtitle&""&date()&"Rss Generator www.actcms.com]]></title>" & _
        ...
        vbcrlf&"  <generator><![CDATA[Rss Generator By ACTCMS.COM]]></generator>" & _
        vbcrlf&"  <image>" & _
...
Function listarticle(cid)
	...
	Set Rs2 = actcms.actexe("SELECT top 50 ID,classid,Title,UpdateTime,ActLink,FileName,InfoPurview,ReadPoint,PicUrl,Intro,Content,CopyFrom,Author,KeyWords  FROM "&ACTCMS.ACT_C(ACTCMS.ACT_L(cid,10),2)&"   Where classid='" & cid & "' AND isAccept=0 AND delif=0   order by IsTop Desc,ID desc" )
	...
	for j=0 to ubound(sql2,2)
		...
		listarticle=listarticle& "<title><![CDATA["&sql2(2,j)&"]]></title>" & vbcrlf
 		If Trim(sql2(9,j))<>"" Then 
		   strdescription=ACTCMS.GetStrValue(Replace(Replace(Replace(AcTCMS.CloseHtml(sql2(9,j)), vbCrLf, ""), "[NextPage]", ""), "&nbsp;", ""), 200)
		Else
		   strdescription=ACTCMS.GetStrValue(Replace(Replace(Replace(AcTCMS.CloseHtml(sql2(10,j)), vbCrLf, ""), "[NextPage]", ""), "&nbsp;", ""), 200)
		End If 
 		Dim strlinkurl,strUserName,strtim,strcategory
 		listarticle=listarticle&"<description><![CDATA["&strdescription&"]]></description>" & vbcrlf
 		listarticle=listarticle&"<link><![CDATA["&AcTCMS.GetInfoUrlall(ACTCMS.ACT_L(cid,10),sql2(1,j),sql2(0,j),sql2(4,j),sql2(5,j),sql2(6,j),sql2(7,j))&"]]></link>" & vbcrlf
		If sql2(12,j)<>"" Then 
		...
		listarticle=listarticle&"<comments><![CDATA["&AcTCMS.GetInfoUrlall(ACTCMS.ACT_L(cid,10),sql2(1,j),sql2(0,j),sql2(4,j),sql2(5,j),sql2(6,j),sql2(7,j))&"]]></comments>" & vbcrlf
