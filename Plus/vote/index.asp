...
<%		Dim id,rs,rootid,rs1,totalren,title,totalcount,h,i,VoteEnd,VoteStart
		...
		If id=0 Then response.write "非法参数":response.end
 	...
 	if not (rs.bof and rs.eof) then
		...
		Set rs1=actcms.actexe("select * from vote_act where rootid="&id&" order by rootid")
 		if rs1.eof  Then response.write "没有这个投票":response.end
			totalcount=0
...
<!-- 
	...
	-->
	<p class="powered">
		Powered by <a href="http://www.actcms.com" title="cms" target="_blank"><strong>ACT<span>CMS</span></strong></a> 
	</p><!-- /powered -->
