...
<% 	Dim UserHS
	...
	if request("a")="del" then 
		Dim TG_ID:TG_ID =Request("ID")
 		IF TG_ID = "" Then
			response.Write "请先选定短信息"
			response.End
		End IF		
 		 TG_ID = Split(TG_ID,",")
		 For I = LBound(TG_ID) To UBound(TG_ID)
 				 Conn.execute("Delete from  Message_Act   where U="& UserHS.UserID &" and  ID = "&ChkNumeric(TG_ID(i))&"")
 		 Next
			call actcms.Alert("删除短信息成功。删除的消息将不可恢复","outbox.asp")
		    Response.End
...
<th width="20%">发送时间</th>
	...
	pages = "page"
   	sql = "SELECT [id],[title],[U],[content],[SendTime]" & _
		" FROM [Message_Act] where Flag=2  and U="& UserHS.UserID & _
		" order by flag,SendTime desc"
 	sqlCount = "SELECT Count([ID])" & _
			" FROM [Message_Act]  where  Flag=2  and U="& UserHS.UserID
 		Dim clsRecordInfo
 ...
 %> 
            ...
            <td  class="align_left">
            <a href="read.asp?A=me&id=<%=arrRecordInfo(0,i)%>"><%=server.htmlencode(arrRecordInfo(1,i))%></a></td>
