...
%>
	...
	<%
	Else
	response.write "┆  <a href=""#"" disabled=""disabled""><s>删除</s></a>"
...
<form id="form1" name="form1" method="post" action="?A=<%= A %>&ModeID=<%= Request.QueryString("ModeID") %>">
  ...
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
    ...
    <tr>
      ...
      <td height="25" class="td_bg"><input <% if A="ESave" then response.Write "disabled" %> name="ModeTable" type="text"  class="Ainput" id="ModeTable" value="<%= ModeTable %>" />
        <% if A<>"ESave" then response.Write "_User_ACT" %>
    ...
    <tr>
	 ...
	 If Rs.EOF  Then
	 	Response.Write	"<option>没有记录,请先添加</option>"
	 Else
		Do While Not Rs.EOF	 %>
          <option value="<%= rs("ID") %>" <%if rs("ID")=SpaceID then response.write " Selected" %>><%= rs("ClassName") %></option>
          <% 
 		Rs.movenext
		Loop
	End if	 %>
	    </select> 
       <span class="h" style="cursor:help;"  onclick="dohelp('regmx_classname')"  id="regmx_classname">帮助</span>     </td>
    </tr>
