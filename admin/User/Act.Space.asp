...
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
  <tr>
	...
	<tr>
      ...
      <span class="h" style="cursor:help;"  onclick="dohelp('space_ClassName')"  id="space_ClassName">帮助</span>  </td>
    </tr>
	<tr>
      <td align="right" ><strong>模板文件名：</strong></td>
	  <td ><input name="ClassTemp" type="text" class="Ainput" id="ClassTemp" value="<%= ClassTemp %>" size="50">
      <span class="h" style="cursor:help;"  onclick="dohelp('space_ClassTemp')"  id="space_ClassTemp">帮助</span>      </td>
	  </tr>
 	 
	<tr>
      <td align="right" ><strong>排序：</strong></td>
	  <td ><input name="ClassOrder" type="text" class="Ainput" id="ClassOrder" value="<%= ClassOrder %>" size="50">
      <span class="h" style="cursor:help;"  onclick="dohelp('space_ClassOrder')"  id="space_ClassOrder">帮助</span>      </td>
	  </tr>
	<tr>
	 ...
	 If Rs.EOF  Then
	 	Response.Write	"<option>没有记录</option>"
	 Else
		Do While Not Rs.EOF	 %>
          <option value="<%= rs("ModeID") %>" <%if rs("ModeID")=ModeID then response.write " Selected" %>><%= rs("ModeName") %>模型</option>
          <% 
 		Rs.movenext
		Loop
	End if	 %>
	    </select>
          <span class="h" style="cursor:help;"  onclick="dohelp('space_ModeID')"  id="space_ModeID">帮助</span> </td>
	  </tr>
 	<tr>
	 ...
	 Else
		Do While Not Rs.EOF	 %>
         <option value="<%= rs("ModeID") %>" <%if rs("ModeID")=UModeID then response.write " Selected" %>><%= rs("ModeName") %></option>
        <% 
 		Rs.movenext
		Loop
	End if	 %></select>   
    <span class="h" style="cursor:help;"  onclick="dohelp('space_UModeID')"  id="space_UModeID">帮助</span>  </td>
    </tr>
     <tr>
