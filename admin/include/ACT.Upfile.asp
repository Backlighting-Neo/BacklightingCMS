...
<% If Not ACTCMS.ChkAdmin() Then  Call Actcms.Alert("对不起，您没有操作权限！","")
	...
	intPageSize =20
	sql = "SELECT [ID],[ArtileID],[UpfileDir],[Extension],[UpdateTime],[ModeID]" & _
		" FROM [Upload_Act]" & _
		" ORDER BY [ID] DESC"
...
<table width="99%" border="0" align="center" cellpadding="1" cellspacing="1" class="table">
   ...
   <%
	...
	'	response.write "<a target=""_blank"" href="&actcms.actsys&"article/typearticle.asp?ModeID="&arrRecordInfo(5,i)&"&ID="&arrRecordInfo(1,i)&">"&Aname("title")&"</a>"
	'Else
