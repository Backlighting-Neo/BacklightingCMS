...
End Function 
	...
	function Classmake
		 Dim Crs
		 Set Crs = Conn.Execute("Select * from Class_act where ParentID='0'   and  actlink<>2 Order by Orderid desc,ID desc")
		 If Not  Crs.Eof Then
		 do while Not Crs.Eof
		 'response.write Crs("ClassID")&"-"
			If   ACTCMS.ACTCMS_QXYZ(Crs("ModeID"),"1",Crs("ClassID")) Then  
			  Response.Write "        d.add(" & Crs("ClassID") & ",0,'" & Crs("ClassName") & "','ACT_Mode/ACT.Manage.asp?Action=ListisAccept&ModeID="&Crs("Modeid")&"&ClassID="&Crs("ClassID")&"','"&Crs("ClassName")&"','main',null,null,null,0,true,'" & Crs("ClassID") & "');" & vbCrLf
			ElseIf  actcms.Ctclass(Crs("ClassID"))=true Then 
			  Response.Write "        d.add(" & Crs("ClassID") & ",0,'无权限','','无权限','main',null,null,null,0,true,'" & Crs("ClassID") & "');" & vbCrLf
			End if
	 ...
	 Function GetChildClassList(ClassID)
	       Dim Sql,Rsc,CheckStr
			...
			If   ACTCMS.ACTCMS_QXYZ(Rsc("ModeID"),"1",Rsc("ClassID")) Then  
				Response.Write "        d.add(" & Rsc("ClassID") & "," & Rsc("ParentID") & ",'" & Rsc("ClassName") & "','ACT_Mode/ACT.Manage.asp?Action=ListisAccept&ModeID="&Rsc("Modeid")&"&ClassID="&Rsc("ClassID")&"','"&Rsc("ClassName")&"','main',null,null,null,0,true,'" & Rsc("ClassID") & "');" & vbCrLf
			ElseIf  actcms.Ctclass(Rsc("ClassID"))=true Then 
				Response.Write "        d.add(" & Rsc("ClassID") & "," & Rsc("ParentID") & ",'无权限','','无权限','main',null,null,null,0,true,'" & Rsc("ClassID") & "');" & vbCrLf
			End if
