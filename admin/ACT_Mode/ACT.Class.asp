...
<table width="98%" border="0" align="center" cellpadding="5" cellspacing="1" class="table">
	...
	<tr>
		<td height="18" ><a href="ACT.Class.asp">管理首页</a>┆<a href="ACT.ClassAdd.asp?Action=add">添加根栏目</a>┆<a href="ACT.ClassAct.asp?Action=one">栏目排序</a></td>
	</tr>
</table>
<table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
	...
	Article_rs.Open "Select Orderid,id,ClassID,ClassName,enname,ClassEName,ActLink,dh,tg,ModeID,makehtmlname from Class_Act where Parentid  = '0'  Order by Orderid asc,ID asc",Conn,1,1
	if Article_rs.eof then
	   Response.Write"<TR  class=""td_bg""><TD colspan=""7""  class=""td_bg"" height=""40"">没有记录</TD></TR>"
	else
	...
	%>
	  ...
	  <td height="22" >
		...
		if Article_rs("ActLink") = "2" then
			Response.Write  "<img src=""../Images/-.gif""></img><a href="&Article_rs("makehtmlname")&"><font color=green>"&Article_rs("ClassName")&"</font></a>"
		ElseIf  Article_rs("ActLink") = "3" Then 
			Response.Write  "<img src=""../Images/-.gif""></img><a href=/"&Article_rs("makehtmlname")&"><b>"&ACTCMS.HTMLCode(Article_rs("ClassName"))&"</b></a>&nbsp;"
		else
			Response.Write  "<img src=""../Images/-.gif""></img><a href=ACT.Manage.asp?Action=t&ModeID="&Article_rs("ModeID")&"&ClassID="&Article_rs("ClassID")&">"&ACTCMS.HTMLCode(Article_rs("ClassName"))&"</a>&nbsp;[内容："&Conn.Execute("Select Count(id) From  "&ACTCMS.ACT_C(Article_rs("ModeID"),2)&"  Where Classid='" & Article_rs("ClassID")&"'")(0)&"]"
		End if
		...
		%>
		  ...
		  <td ><div align="center">
			  ...
			  if Article_rs("tg")=1 then
				 response.Write "<font color=red title='该栏目可以投稿'>稿</font>&nbsp;┆&nbsp;" 
			  Else
				 response.Write "<strike title='该栏目不可以投稿'>稿</strike>&nbsp;┆&nbsp;" 
			  End if
			  if Article_rs("dh")=1 then
				 response.Write "<font color=red title='在导航中显示'>显示</font>&nbsp;" 
			  Else
				 response.Write "<font title='在导航中隐藏'>隐藏</font>&nbsp;" 
			  End if
	  %>
				</div></td>
			<td  >
				<div align="center">
				 <a  target="_blank" href="../../List.asp?L-<% = Article_rs("ClassID")%>.Html" >预览</a>┆<a title="添加内容" href="ACT.Add.asp?ModeID=<% = Article_rs("ModeID")%>&ClassID=<% = Article_rs("ClassID")%>" >内容</a>┆<a title="添加子栏目" href="ACT.ClassAdd.asp?ModeID=<% = Article_rs("ModeID")%>&ClassID=<% = Article_rs("ClassID")%>&Action=add">子栏目</a>┆<a href="ACT.ClassAdd.asp?ClassID=<% = Article_rs("ClassID")%>&Action=edit">修改</a>┆<a href="ACT.Action.asp?ModeID=<% =Article_rs("ModeID")%>&ClassID=<% = Article_rs("ClassID")%>&Action=del"  onClick="{if(confirm('确定删除您所选择的栏目吗?\n\n此栏目下的所有信息也将被删除,此操作不可恢复!!!')){return true;}return false;}">删除</a>
	...
	Public Function GetClassValue(TypeID,CompatStr)  
		...
		Dim tg,dh,ClassRs
		Set ClassRs = Conn.Execute("Select id,dh,orderid,ClassName,tg,ClassEName,enname,ClassID,ActLink,ModeID,makehtmlname from Class_Act where ParentID='" & TypeID & "'   Order by Orderid asc,ID asc" )
		...
		do while Not ClassRs.Eof
			...
			GetClassValue = GetClassValue & "<td class=""td_bg"" align=""center""><div align=""center""><a  target=""_blank"" href=""../../List.asp?L-"&ClassRs("ClassID")&".Html"" >预览</a>┆<a title=""添加内容"" href=""ACT.Add.asp?ModeID="&ClassRs("ModeID")&"&ClassID="&ClassRs("ClassID")&""" >内容</a>┆<a title=""添加子栏目"" href=""ACT.ClassAdd.asp?ModeID="&ClassRs("ModeID")&"&ClassID="&ClassRs("ClassID")&"&Action=add"">子栏目</a>┆<a href=""ACT.ClassAdd.asp?ClassID="&ClassRs("ClassID")&"&Action=edit"">修改</a>┆<a href=""ACT.Action.asp?ModeID="&ClassRs("ModeID")&"&ClassID="&ClassRs("ClassID")&"&Action=del""   onClick=""{if(confirm('确定删除您所选择的栏目吗？\n\n此栏目下的所有信息也将被删除,此操作不可恢复!!!')){return true;}return false;}"">删除</a>"&vbNewLine&"<input name=""ClassID"" type=""checkbox"" id=""ClassID"" value="""& ClassRs("ClassID")&"""></div></td>" & Chr(13) & Chr(10)
