...
Dim Hits,Keywords,CopyFrom,UpdateTime,TypeTitleFont,rs3
...
Dim Tmps,TmpUs,url
 Dim ChargeType,InfoPurview,arrGroupID,ReadPoint,PitchTime,ReadTimes,DividePercent
	...
	IF Request("Action") = "add" Then '添加
			...
			If IsArray(IF_NULL) Then
 				For I=0 To Ubound(IF_NULL,2)
					...
					ElseIf IF_NULL(4,I)="1" Then 
 							 Save_SQL("" & IF_NULL(0,I) & "")= actcms.AField(IF_NULL(5,I))
		...
		'============添加管理员日志=====================
		Dim ShortDescription,LongDescription
		ShortDescription="添加文章"
		LongDescription="【" & ID & "】" & Title 
	...
	End Function
 	If CLng(StrMakeHtml) = 1  Then '立即生成Html
			...
			If ACTCMS.ACT_C(ModeID,3) = 0 Or ACTCMS.ACT_C(ModeID,3) = 2 Or readpoint>0 or infopurview="2" Or (infopurview=0 And (ACTCMS.ACT_L(ClassID,19)=1 Or ACTCMS.ACT_L(ClassID,20)>1 Or ACTCMS.ACT_L(ClassID,19)=2)) And actlink=0 Then 
				URL=ACTCMS.ActCMS_Sys(2)&ACTCMS.ActSys&"List.asp?C-"&ModeID&"-"&rs("ID")&".Html"
			Else 
				If actlink = 0 Then
 					  Response.Write "<div align=center><iframe src=""../Include/ACT.C.asp?ModeID="&ModeID&"&RefreshFlag=ID&ID=" & rs("ID") &""" width=""100%"" height=""40"" frameborder=""0""></iframe></div>"
					Dim PrevArticle
				...
				If ACTCMS.ACT_C(ModeID,9) = 2 Then 
					  ...
					  For I=0 To Ubound(FolderIDArr)
						response.Write "<div align=center><iframe src=""../Include/ACT.L.asp?ModeID="&ModeID&"&RefreshFlag=IDS&CID=" & FolderIDArr(i) &""" width=""0"" height=""0"" frameborder=""0""></iframe></div>"
					  Next
				End If
			End If
