...
<form id="form1" name="form1" method="post" action="?A=<%= A %>&ModeID=<%= Request.QueryString("ModeID") %>">
  ...
  <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
    ...
    <tr>
      ...
      <td height="25"><input   class="Ainput"  <% if A="ESave" then response.Write "disabled" %> name="ModeTable" type="text" id="ModeTable" value="<%= ModeTable %>" />
    ...
    <tr>
      ...
      <td height="25">
	  <input     <% IF IsMail = 0 Then Response.Write "Checked" %> id="IsMail1" type="radio" name="IsMail" value="0">
        <label for="IsMail1">启用</label>
      <input    <% IF IsMail = 1 Then Response.Write "Checked" %> id="IsMail2"  type="radio" name="IsMail" value="1"> 
     ...
     <label for="UnlockTime2">不启用</label></td>
	    ...
	    <tr  id="times1"	<%If UnlockTime<>0 Then response.write "style=""DISPLAY: none"""%>
>
      ...
      <td height="25"><input  class="Ainput"  size=30 name="StartTime" type="text" id="StartTime" value="<%= StartTime %>"  onClick="WdatePicker();" />
	    ...
	    <tr id="times2" <%If UnlockTime<>0 Then response.write "style=""DISPLAY: none"""%> >
