...
If Action="save" Then
 	Echo "<p id=""loading2"">正在写入插件信息, 请稍候...  如果长时间停止响应, 请 <a href=""javascript:window.location.reload();"" title=""点此重试"">[点此重试]</a></p>"
 	Dim Pack_Error
	Pack_Error=0

	If SelectedTheme="" Then
		Pack_Error= "<font color=""red"">  插件的名称为空.</font>"
  	Else
	...
	If Pack_Error = "0" Then
 		Call Actcms.ActErr("插件信息保存完成","Sys_Act/plugin/Act.Theme.Type.asp?theme="& Server.URLEncode(SelectedTheme) &"","")
 	Else
  		Call Actcms.ActErr("插件信息保存失败"&Pack_Error,"","1")
 	End If
...
'On Error Resume Next
	...
	Set fso = CreateObject(ACTCMS.FsoName)
 		If fso.FileExists(Tpath & ThemePath & SelectedTheme & "/" & "theme.xml") Then
			...
			If IsObject(XML) And XML.readyState=4 And XML.parseError.errorCode = 0 Then
					...
					ThemeModified=GetNode("modified"):ThemeDescription=GetNode("description"):themeadminurl=GetNode("themeadminurl")
 ...
 %>
  ...
  <form method="post" action="Act.Theme.Edit.asp?act=save&theme=<%=SelectedTheme%>">
    ...
    <tr>
      <td height="25" align="right">插件ID&nbsp;&nbsp;</td>
    ...
    <tr>
      <td height="25" align="right">插件名称&nbsp;&nbsp;</td>
    ...
    <tr>
      <td height="25" align="right">后台管理地址&nbsp;&nbsp;</td>
      <td height="25"><input  class="Ainput"   name="themeadminurl" type="text" size="30" value="<%=themeadminurl%>" />
       </td>
    </tr>
    ...
    <tr>
      <td height="25" align="right">插件的发布页面&nbsp;&nbsp;</td>
      <td height="25"><input  class="Ainput"   name="ThemeURL" type="text" size="60" value="<%=ThemeURL%>" />
      (带 http:// 等协议名的页面地址, 以方便使用者获取更多的插件信息) </td>
    </tr>
     
    <tr>
      <td height="25" align="right">插件简介&nbsp;&nbsp;</td>
      <td height="25">
    ...
    <tr >
      <td height="25"   colspan="2"  class="bg_tr" align="center"> 
       以下信息对查找插件可用更新极为重要, 建议在每次修改插件后更新这些信息</td>
    </tr>
 
    <tr>
      <td height="25" align="right">插件的版本号&nbsp;&nbsp;</td>
      <td height="25"><input  class="Ainput"   name="ThemeVersion" type="text" size="30" value="<%=ThemeVersion%>" />
      </td>
    </tr>
 
    <tr>
      <td height="25" align="right">您的插件的发布日期&nbsp;&nbsp;</td>
 ...
 <tr>
      ...
      <input type="button" name="Submit2"  onclick="self.location.href='Index.asp';" class="ACT_btn" value="  取消并返回插件管理  " /></td>
    </tr>
...
'On Error Resume Next
	...
	Set XmlDoc = Server.CreateObject("Microsoft.XMLDOM")
		...
		Set themeadminurl = XmlDoc.SelectSingleNode("//theme").AppendChild(XmlDoc.CreateElement("themeadminurl"))
			themeadminurl.Text = Replace(Replace(Request.Form("themeadminurl"),vbCr,""),vbLf,"")
		Set themeadminurl=Nothing
