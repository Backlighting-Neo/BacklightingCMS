...
<body>
 <table width="98%" border="0" align="center" cellpadding="2" cellspacing="1" class="table">
  <tr>
    <td class="bg_tr">您现在的位置：主题样式 &gt;&gt; 查看主题的可用更新 </td>
  </tr>
  <tr>
    <td>
  <div class="md-head" >
...
If Action="" Then
	...
	Echo "<div class=""top1""><span style="" margin-left:10px"" ><strong>提示：</strong> 以下列出了您需要更新的主题:</span></div>"
	Echo "<p class=""status-box"" id=""notfound"" style=""display:none;"">!! 暂时没有找到需要更新的主题.</p>"
	Response.Flush
	...
	For Each f1 in fc
		...
		'	XmlDataLocal=(LoadFromFile(Tpath & ThemePath & f1.name & "/Theme.xml","utf-8"))
		'	echo Tpath & ThemePath & f1.name & "\" & "Theme.xml<br />"
		...
		'		XmlDataWeb=(LoadFromFile(Tpath & ThemePath & f1.name & "/" & "Update.xml","utf-8"))
		 echo "<table width=""98%"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""1"" class=""table""> <tr><td class=""bg_tr"">查看主题</td></tr><tr><td   colspan=""2"" >"

				Echo "<p class=""download-box"">"
				Echo "主题: <b>"& Item_Name_Local &"</b> <b class=""notice"">发现可用的新版本!</b><br/><br/>"

				Echo "当前主题版本为: <b>"& Item_Version_Local &"</b>. 发布日期为: <b>"& Item_PubDate_Local &"</b>. 最后修改日期为: <b>"& Item_Modified_Local &"</b>.<br/>"
				...
				If Item_Url_Web<>Empty Then
					Echo "<a href="""& Item_Url_Web &""" target=""_blank"" title=""查看主题的发布页面"">[点此查看主题的发布信息!]</a> "
				End If

				Echo "<a class=""notice"" href=""Act.Theme.Install.Asp?act=confirm&amp;url=" & Server.URLEncode(Update_URL & f1.name) & """ title=""升级此主题"">[升级此主题]</a>"
...
If Action="clear" Then
	...
	Echo "<span id=""found"" style=""display:none;"">!! 已清除以下主题的更新提示:</span>"
	...
	For Each f1 in fc
 		If fso.FileExists(Tpath & ThemePath & f1.name & "/" & "Theme.xml") Then
			...
			echo "<table width=""98%"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""1"" class=""table""> <tr><td class=""bg_tr"">检测    <b class=red>" & Item_Name_Local & "</b></td></tr><tr><td   colspan=""2"" >"
				...
				Echo "主题: <b>"& Item_Name_Local &"</b> <b class=""allright"">的新版提示已清除!</b>"
				Echo "</p>"
				Response.Flush
				 echo"</td></tr> </table>"
				m=True
			...
			echo "<table width=""98%"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""1"" class=""table""> <tr><td class=""bg_tr"">检测    <b class=red>" & Item_Name_Local & "</b></td></tr><tr><td   colspan=""2"" >"
				...
				Echo "<p class=""status-box"">"
				Echo "主题: <b>"& Item_Name_Local &"</b> <b class=""allright"">的不支持更新提示已清除!</b>"
				Echo "</p>"
				Response.Flush
				 echo"</td></tr> </table>"
				m=True
...
If Action="check" Then
	...
	Echo "<div class=""top1""><span style="" margin-left:10px"" ><strong>提示：</strong> 查找更新的过程会消耗一些时间, 时间长短会因您安装的主题数量而异, 请耐心等候..</span></div>"
	Response.Flush
 	i=0
	...
	'	Set objXmlVerChk=New ThemeSapper_CheckVersionViaXML
		...
		If fso.FileExists(Tpath & ThemePath & f1.name & "/" & "Theme.xml") Then
			...
			If i>=n Then
				Echo "<p   id=""checking"">>>> 主题: " & Item_Name_Local & " 正在检查中...</p>"
				Response.Flush
			Else
				If fso.FileExists(Tpath & ThemePath & f1.name & "/" & "verchk.xml") Then
					t="<b class=""notice""> 发现新版本!</b><a class=""notice"" href=""Act.Theme.Install.Asp?act=update&amp;url=" & Server.URLEncode(Update_URL & f1.name) & """ title=""升级主题""><b >升级主题!</b></a>"
				ElseIf fso.FileExists(Tpath & ThemePath & f1.name & "/" & "error.log") Then
					t="<span class=""red"">该主题不支持在线更新.</span>"
				Else
					t="<span class=""green"">未发现新版本.</span>"
				End If

				Echo ">>> 主题: " & Item_Name_Local & "  "& t & ""
				Response.Flush
			End If
			...
			Call ActCMS.CreateFolder(actcms.ActSys&actcms.SysThemePath&"/"&f1.name &"/")
  		...
  		'    ActCMS.DeleteFile(Tpath&ThemePath&Install_ID& "\Update.xml")
			...
			If (DateDiff("d",Item_PubDate_Web,Item_PubDate_Local)>0 Or DateDiff("d",Item_Modified_Web,Item_Modified_Local)>0) Then
				 Call ActCMS.DeleteFile(Tpath&ThemePath&f1.name& "\verchk.xml")
			...
			Else
				 Call ActCMS.DeleteFile(Tpath&ThemePath&f1.name& "\verchk.xml")
			End If
			If (Item_ID_Web<>Item_ID_Local) Then Call ActCMS.DeleteFile(Tpath&ThemePath&f1.name& "\verchk.xml")

			 

				If fso.FileExists(Tpath & ThemePath & f1.name & "/" & "verchk.xml") Then
					t="<b class=red>" & Item_Name_Local & "</b> <b class=""notice"">检查完毕,发现新版本!</b><a class=""notice"" href=""Act.Theme.Install.Asp?act=update&amp;url=" & Server.URLEncode(Update_URL & f1.name) & """ title=""升级主题""><b >发现该主题的新版本!</b></a>"
				ElseIf fso.FileExists(Tpath & Item_Name_Local & f1.name & "/" & "error.log") Then
					t="<b class=red>" & Item_Name_Local & "</b> <b class=""notice"">检查完毕,该主题不支持在线更新.</b>"
				Else
	...
	If m Then
		echo "<table width=""98%"" border=""0"" align=""center"" cellpadding=""2"" cellspacing=""1"" class=""table""> <tr><td class=""bg_tr"">所有主题已检查完成</td></tr><tr><td   colspan=""2"" >"
 		Echo "<p><input onclick=""window.scrollTo(0,0);"" type=""button""  class=""ACT_btn""   value=""返回页面顶部""  title=""返回页面顶部"" /> <input onclick=""self.location.href='Act.Theme.Check.asp';"" type=""button""  class=""ACT_btn""  value=""查看需要更新的主题"" title=""查看更新结果"" />"
