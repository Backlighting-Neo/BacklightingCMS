	 ...
	 Set ACTCode=Nothing:Set ACTCMS=Nothing
		...
		If R_Sql <> "" Then
			...
			If R_RS.EOF And R_RS.BOF Then
				...
				.Write "txt2.innerHTML=""没有可生成的单页！<br><br><input name='button1' type='button' onclick=javascript:location='Index.asp'; class='button' value=' 返 回 '>"";" & vbCrLf
			...
			Else
 				...
 				For NowNum=1 To MaxNum
				  ...
				  FsoHtmlList="<div align=center><li>标题为<font color=red>"  & R_RS("pageName") & "</font>已生成</div>"
					...
					Application(AcTCMSN & "ACTCMSTCJ")=R_RS("pagename")
 					Application(AcTCMSN & "ACTCMS_TCJ_Type") = "OTHER"
					...
					TemplateContent = ACTCode.LoadTemplate(R_RS("tempurl"))
					End If 
					IF TemplateContent <> "" Then
						TemplateContent = ACTCode.LabelReplaceAll(TemplateContent)
					End If 
					If InStr(TemplateContent, "{$diycontent}") > 0   Then
						TemplateContent = Replace(TemplateContent, "{$diycontent}", R_RS("content"))
					End If 

					If InStr(TemplateContent, "{$pagename}") > 0  And  R_RS("pagename")<>""  Then
						TemplateContent = Replace(TemplateContent, "{$pagename}", R_RS("pagename"))
					Else
						TemplateContent = Replace(TemplateContent, "{$pagename}", "")
					End If 
					TemplateContent=ACTCode.actcmsexe(TemplateContent)
 					Call ACTCode.FSOSaveFile(TemplateContent,FileName)				
			...
			Next
				...
				.Write "txt2.innerHTML=""生成单页结束！100"";" & vbCrLf
